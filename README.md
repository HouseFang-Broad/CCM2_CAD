# Role of CCM2 in coronary atherosclerosis

This repository contains analysis code accompanying the manuscript on CCM2 heterozygous knockout and atherosclerosis development, integrating single-cell RNA sequencing and cell painting assays.
A second part of this repository is focused around understanding the impact of CCM2 common coding variant V53I using variant painting. 


## scRNA-seq

Single-cell RNA sequencing of aortic tissue from CCM2 heterozygous knockout and wild-type mice at Week 0 and Week 12. See [`scRNAseq/README.md`](scRNAseq/README.md) for details.

Raw data: [GEO GSE315884]

## Cell Painting

Morphological profiling using the Cell Painting assay in two in vitro systems (U2OS and teloHAECs). 
U2OS: [cell_painting/U2OS/README.md](cell_painting/U2OS/README.md)
teloHAEC: [cell_painting/teloHAEC/](cell_painting/teloHAEC/)